...
/**
 * 3) Crea el proyecto LibroUnAutor, que estar� formado por las siguientes
 ...
 * 
 * El m�todo toString devolver� la siguiente cadena de caracteres (por ejemplo):
 ...
 * 
 * Supondremos que un libro s�lo tiene un autor y que el m�todo toString
 * devolver� una cadena de caracteres como la siguiente (por ejemplo): �Libro
 ...
 * cantidad = 3]� Escribe la clase TestLibroAutor, que adem�s del m�todo main
 * deber� tener otros m�todos est�ticos para que al ejecutar la aplicaci�n
 * muestre un men� con las siguientes opciones: 1- Crear libros. (Esta opci�n
 * preguntar� al usuario cu�ntos libros quiere crear y a continuaci�n proceder�
 * a instanciarlos) 2- Modificar autor. 3- Modificar libro. 4- Listado de
 * libros. 5- Listado de autores. 0- Salir.
 * 
 * 4) Crea un nuevo proyecto, al que llamar�s LibroVariosAutores que utilice las
 * clases del ejercicio anterior y modifica la clase Libro para que un libro
 * pueda tener varios autores. El m�todo toString debe devolver, por ejemplo:
...
public class Main {
	 ...
	 */
	/**
	 * public static Autor[] IntroducirAutores() { String nombre; String email;
	 * char sexo; Fecha fechaNacimiento; Autor autor[] = new Autor[5]; int
	 * numeroAutores = Leer.pedirEntero("introduce numero de autores"); for (int
	 * j = 0; j < numeroAutores; j++) { nombre = Leer.pedirCadena("Introduce el
	 * nombre del autor"); email = Leer.pedirCadena("email"); sexo =
	...
	public static Autor[] variosAutores(Autor autores[]) {
		...
		autores[Autor.getSiguiente()] = new Autor("Patrick S�skind", "patrick@fontecabras.es", 'M',
				new Fecha(24, 1, 1987));
	...
	public static Libro[] variosLibros(Autor autores[], Libro libros[]) {
		...
		libros[Libro.getContador()] = new Libro("El se�or de los anillos", autores[1]);
		...
		libros[Libro.getContador()] = new Libro("En la boca del drag�n", autores[3]);
